...
// อ่าน environment variables จาก .env.local หรือ .env
function loadEnv() {
...
const dbName = env.MONGODB_DB || process.env.MONGODB_DB || "sciencehome";

// แก้ไข URI หากมีอักขระพิเศษใน password ที่ไม่ได้ encode
if (uri) {
  // ตรวจสอบและ encode password ใน connection string
  ...
  if (uriMatch) {
    ...
    // เพิ่ม authSource=admin ถ้ายังไม่มี
...
async function testConnection() {
  ...
  try {
    ...
    try {
      ...
      console.log(`   (ไม่สามารถอ่าน server status ได้ - อาจไม่มีสิทธิ์)\n`);
    }
